...
async def connect_and_run(token: str) -> None:
    ...
    async with streamablehttp_client(
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            # idempotency_key (Story 41-1): generate the key ONCE and reuse it
            # verbatim on any retry — if the original call already landed, the
            # retry replays that post instead of creating a duplicate, e.g.:
            #     await session.call_tool("post_message",
            #         {"content": ..., "idempotency_key": post_key})  # safe retry
            post_key = str(uuid.uuid4())
            result = await session.call_tool(
                "post_message",
                {
                    "content": "Hello from the Python MCP sample!",
